...
private:
  ...
  static void probe_entire_mesh(const xy_pos_t &near, const bool do_ubl_mesh_map, const bool stow_probe, const bool do_furthest, uint8_t color_sp_offset) __O0;
...
public:
  ...
  static int8_t cell_index_x_raw(const_float_t x) {
    return FLOOR((x - (lcd_rts_settings.probe_margin_x)) * RECIPROCAL(get_mesh_x_dist()));
  }

  static int8_t cell_index_y_raw(const_float_t y) {
    return FLOOR((y - (lcd_rts_settings.probe_margin_y_front)) * RECIPROCAL(get_mesh_y_dist()));
  }

  static bool cell_index_x_valid(const_float_t x) {
    return WITHIN(cell_index_x_raw(x), 0, (max_points.x - 1) - 1);
  }

  static bool cell_index_y_valid(const_float_t y) {
    return WITHIN(cell_index_y_raw(y), 0, (max_points.y - 1) - 1);
  }

  static uint8_t cell_index_x(const_float_t x) {
    return constrain(cell_index_x_raw(x), 0, (max_points.x - 1) - 1);
  }

  static uint8_t cell_index_y(const_float_t y) {
    return constrain(cell_index_y_raw(y), 0, (max_points.y - 1) - 1);
  }
  ...
  static int8_t closest_x_index(const_float_t x, const xy_uint8_t &_max_points) {
    const int8_t px = (x -  (lcd_rts_settings.probe_margin_x) + (get_mesh_x_dist()) * 0.5) * RECIPROCAL(get_mesh_x_dist());
    return WITHIN(px, 0, (max_points.x) - 1) ? px : -1;
  }
  static int8_t closest_y_index(const_float_t y, const xy_uint8_t &_max_points) {
    const int8_t py = (y - (lcd_rts_settings.probe_margin_y_front) + (get_mesh_y_dist()) * 0.5) * RECIPROCAL(get_mesh_y_dist());
    return WITHIN(py, 0, (max_points.y) - 1) ? py : -1;
  }

  static xy_int8_t closest_indexes(const xy_pos_t &xy) {
    return { closest_x_index(xy.x, max_points), closest_y_index(xy.y, max_points) };
  }
   ...
   */
  static float z_correction_for_x_on_horizontal_mesh_line(const_float_t rx0, const uint8_t x1_i, const int yi, const xy_uint8_t &_max_points) {
    if (!WITHIN(x1_i, 0, (max_points.x) - 1) || !WITHIN(yi, 0, (max_points.y) - 1)) {

      if (DEBUGGING(LEVELING)) {
        if (WITHIN(x1_i, 0, (max_points.x) - 1)) DEBUG_ECHOPGM("yi"); else DEBUG_ECHOPGM("x1_i");
    ...
    return z1 + xratio * (z_values[_MIN(x1_i, (max_points.x) - 2) + 1][yi] - z1);  // Don't allow x1_i+1 to be past the end of the array
  ...
  //
  static float z_correction_for_y_on_vertical_mesh_line(const_float_t ry0, const int xi, const int y1_i, const xy_uint8_t &_max_points) {
    if (!WITHIN(xi, 0, (max_points.x) - 1) || !WITHIN(y1_i, 0, (max_points.y) - 1)) {

      if (DEBUGGING(LEVELING)) {
        if (WITHIN(xi, 0, (max_points.x) - 1)) DEBUG_ECHOPGM("y1_i"); else DEBUG_ECHOPGM("xi");
    ...
    return z1 + yratio * (z_values[xi][_MIN(y1_i, (max_points.y) - 2) + 1] - z1);  // Don't allow y1_i+1 to be past the end of the array
                                                                                        // If it is, it is clamped to the last element of the
                                                                                        // z_values[][] array and no correction is applied.
  }
  ...
  static float get_z_correction(const_float_t rx0, const_float_t ry0) {
     ...
     */
    #ifdef UBL_Z_RAISE_WHEN_OFF_MESH
      if (!WITHIN(rx0, lcd_rts_settings.probe_margin_x, X_BED_SIZE - lcd_rts_settings.probe_margin_x) || !WITHIN(ry0, lcd_rts_settings.probe_margin_y_front, Y_BED_SIZE - lcd_rts_settings.probe_margin_y_back))
        return UBL_Z_RAISE_WHEN_OFF_MESH;
    #endif

    uint8_t mx = _MIN(cx, (max_points.x) - 2) + 1, my = _MIN(cy, (max_points.y) - 2) + 1;
